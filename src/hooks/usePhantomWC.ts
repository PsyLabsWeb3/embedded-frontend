...
type PhantomWC = {
  ...
  signMessage: (message: Uint8Array | string) => Promise<string>;                 // base64
  signTransaction: (tx: Transaction | VersionedTransaction | Uint8Array | string) => Promise<string>; // base64
  signAndSendTransaction: (connection: Connection, tx: Transaction | VersionedTransaction | Uint8Array | string) => Promise<string>; // base58
};
...
function txToBase64(tx: Transaction | VersionedTransaction | Uint8Array | string): string {
  ...
  // @ts-ignore serialize existe en ambos tipos
...
function getAddressFromSession(session: SessionTypes.Struct | null): string | null {
  ...
  return accs[0].split(':')[2] || null; // "solana:<net>:<pubkey>"
}

function openPhantomWithWCURI(uri: string) {
  try {
    window.location.href = `wc:${encodeURIComponent(uri)}`;
  } catch {
...
export function usePhantomWC(opts: UsePhantomWCOptions): PhantomWC {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (uri) openPhantomWithWCURI(uri);

      const sess = await approval(); // se resuelve cuando el usuario aprueba en Phantom
      setSession(sess);
    } finally {
      setConnecting(false);
    }
  }, [chainId, opts.projectId]);
  ...
  return { address, connected, connecting, chainId, connect, disconnect, signMessage, signTransaction, signAndSendTransaction };
}
