import styles from "./PositionNumber.module.css";

interface PositionNumberProps {
  position: number;
}

// Trophy icon SVG component
const TrophyIcon: React.FC<{ isTop3: boolean }> = ({ isTop3 }) => (
  <svg
    width="20"
    height="20"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={styles.trophyIcon}
  >
    <path
      d="M19 5H17V3H7V5H5C3.9 5 3 5.9 3 7V8C3 10.55 4.92 12.63 7.39 12.94C8.02 14.44 9.37 15.57 11 15.9V19H7V21H17V19H13V15.9C14.63 15.57 15.98 14.44 16.61 12.94C19.08 12.63 21 10.55 21 8V7C21 5.9 20.1 5 19 5ZM5 8V7H7V10.82C5.84 10.4 5 9.3 5 8ZM19 8C19 9.3 18.16 10.4 17 10.82V7H19V8Z"
      fill={isTop3 ? "#5dd62c" : "#959698"}
    />
  </svg>
);

export const PositionNumber: React.FC<PositionNumberProps> = ({ position }) => {
  const isTop3 = position <= 3;

  return (
    <div className={styles.positionContainer}>
      <span
        className={`${styles.positionNumber} ${isTop3 ? styles.positionTop3 : styles.positionDefault}`}
      >
        #{position}
      </span>
      <TrophyIcon isTop3={isTop3} />
    </div>
  );
};
